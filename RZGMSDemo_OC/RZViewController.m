...
#import <RZGMS/RZGMSKit.h>
#import "RZTableViewCell.h"
#import "NSString+Util.h"


@interface RZViewController () <RZGMSDelegate, UITableViewDelegate, UITableViewDataSource, RZGMSChannelDelegate>

@property (nonatomic, strong) RZGMSKit *rzGMSKit;
@property (nonatomic, strong) RZGMSChannel *gmsChannel;
...
- (void)viewDidLoad
{
    ...
    [_channelTableView registerNib:[UINib nibWithNibName:@"RZTableViewCell" bundle:nil] forCellReuseIdentifier:@"RZTableViewCell"];

    _peerTableView.delegate = self;
    _peerTableView.dataSource = self;
    [_peerTableView registerNib:[UINib nibWithNibName:@"RZTableViewCell" bundle:nil] forCellReuseIdentifier:@"RZTableViewCell"];

    //换成你自己的appid
    NSString *appId = @"appId";
    _rzGMSKit = [[RZGMSKit alloc] initWithAppId:appId delegate:self];
...
- (IBAction)onClickJoin:(id)sender
{
    ...
    if (_gmsChannel == nil) {
        _gmsChannel = [self.rzGMSKit createChannelWithId:channel delegate:self];
    }
    __weak typeof(self) weakSelf = self;
...
- (IBAction)onClickSend:(id)sender
{
    ...
    RZGMSMessage *message = [[RZGMSMessage alloc] initWithText:text];
...
- (void)sendPeerMessage:(RZGMSMessage *)message
{
    ...
    [self.rzGMSKit sendMessage:message toPeer:peerId sendMessageOptions:options completion:^(RZGMSSendPeerMessageErrorCode errorCode) {
        
    }];
}
- (void)sendChannelMessage:(RZGMSMessage *)message
{
    RZGMSSendMessageOptions *options = [[RZGMSSendMessageOptions alloc] init];
    options.enableOfflineMessaging = true;
...
{
    RZTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"RZTableViewCell"];
    NSDictionary *dict;
    if (tableView == _channelTableView) {
...
#pragma mark - RZGMSChannel
- (void)channel:(RZGMSChannel *)channel messageReceived:(RZGMSMessage *)message fromMember:(RZGMSMember *)member {
    NSDictionary *dict = @{
        @"name":member.userId,
        @"value":message.text,
    };
