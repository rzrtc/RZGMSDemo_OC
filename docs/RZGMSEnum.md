...
typedef NS_ENUM(NSInteger, RZGMSConnectionState) {
    ...
    RZGMSConnectionStateAborted = 5,      //被踢出，终止
};
```

### RZGMSConnectionChangeReason
```objectivec
typedef NS_ENUM(NSInteger, RZGMSConnectionChangeReason) {
    ...
    RZGMSConnectionChangeReasonRemoteLogin = 8,    //远端登录，本地被踢出
};
```
### RZGMSLoginErrorCode
```objectivec
...
## Channel
### RZGMSJoinChannelErrorCode
```objectivec
typedef NS_ENUM(NSInteger, RZGMSJoinChannelErrorCode) {
    RZGMSJoinChannelErrorOk = 0,               //用户成功加入频道
...
```
### RZGMSChannelMemberCountErrorCode
```objectivec
...
```
### RZGMSSendChannelMessageErrorCode
```objectivec
...
```
## User
### RZGMSProcessAttributeErrorCode
```objectivec
...
### RZGMSQueryPeersOnlineErrorCode
```objectivec
typedef NS_ENUM(NSInteger, RZGMSProcessAttributeErrorCode) {
    RZGMSAttributeOperationErrorOk = 0,                //操作成功
    RZGMSAttributeOperationErrorFailure = 2,           //操作失败
...
### RZGMSPeerSubscriptionStatusErrorCode
```objectivec
...
```
### RZGMSInvitationApiCallErrorCode
```objectivec
